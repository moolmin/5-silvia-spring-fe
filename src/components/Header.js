...
import { FaLock, FaUserEdit, FaKey, FaSignOutAlt } from "react-icons/fa";
...
function Header({ showBackButton, showUserProfile }) {
  ...
  const userEmail = isLoggedIn ? localStorage.getItem("email") : null;
  const { profileImage, nickname, userId } = useUserProfile(userEmail);
  const navigate = useNavigate();
  const defaultProfileImage =
    "https://lh3.google.com/u/0/d/1ra2p2F4dl1ITC1r3M2ORKyqjt-O00EgE=w3024-h1714-iv2";
  ...
  const handleLogout = async (event) => {
    ...
    try {
      await fetchWithToken(`${process.env.REACT_APP_API_ENDPOINT}/api/logout`, {
        method: "POST",
        credentials: "include",
      });
    ...
    <div className="Header">
      <div className="HeaderContent">
        <div className="HeaderEmptyBox">
          {showBackButton && (
            <button className="BackBtn" onClick={handleBackButtonClick}>
              〈
            </button>
          )}
        </div>
            ...
            <div className="UserProfile" onClick={toggleDropdown}>
              ...
              >
                {isLoggedIn ? (
                  <>
                    <a
                      href={userId ? `/profile/edit/${userId}` : "#"}
                      className="dropboxUserInfo"
                    >
                    ...
                    <a
                      href={userId ? `/users/${userId}/password` : "#"}
                      style={{
                  ...
                  >
                    <FaLock />
                    <span style={{ margin: "0 0 0 8px" }}>로그인</span>
                  </a>
                )}
              </div>
            </div>
